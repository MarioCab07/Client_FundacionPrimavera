    ...
    <>
      ...
      <article className="flex flex-col gap-4 py-12">
        <div className="flex gap-4 items-center justify-between ">
          ...
          <p>
            Genero: <span className="font-semibold">{vol.gender}</span>
          </p>
          <p>
        ...
        <div className="flex gap-2 justify-center items-center w-full ">
          <div className="flex flex-col gap-2 flex-1 items-start h-max">
            <h4 className="font-bold">Información de Contacto</h4>

            <p>
              Telefono:{" "}
              <span className="font-semibold">{vol.phone_number}</span>
            </p>
            <p>
              Direccion: <span className="font-semibold">{vol.address}</span>
            </p>
          </div>
          <div className="flex flex-col gap-2 flex-1 items-start h-max">
            <h4 className="font-bold">Información Profesional</h4>
            ...
            <p>
              Universidad:{" "}
              <span className="font-semibold">{vol.university}</span>
            </p>
            <p>
              Año estudiado:{" "}
              <span className="font-semibold">{vol.year_studied}</span>
            </p>
          </div>
          ...
          <div className="flex flex-col gap-2 flex-1 items-start">
            ...
            <p>
              Tipo de Servicio:{" "}
              <span className="font-semibold">{vol.service_type}</span>
            </p>
            <p>
              Fecha de Inicio:{" "}
              <span className="font-semibold">
              ...
              <div className="flex flex-col gap-2 flex-1 items-start h-max">
                ...
                <p>
                  {vol.userName !== "NA" ? (
                    <>
                      Usuario:{" "}
                      <span className="font-semibold">{vol.userName}</span>
                    </>
                  ) : (
                    <>
                      <span className="font-semibold">
          ...
          {isAdmin ? (
            <>
              ...
              {vol.active ? (
                ...
                <button
                  className="bg-lime-400 rounded-2xl p-3 w-fit flex gap-4 items-center hover:bg-green-50 transition-all duration-500 ease-in-out cursor-pointer"
                  style={{ boxShadow: "rgba(0, 0, 0, 0.24) 0px 3px 8px" }}
                  onClick={() => {
...
const VolDetails = ({ vol, setVolSelected, fetchData }) => {
  ...
  max-w-fit mx-auto absolute top-2/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border-amber-300 border-2 border-solid min-h-full`}
      >
        {!showModify &&
        !showDeactive &&
        !showReactive &&
